...
contract UstxDEXv2 is Initializable {
	...
	*/
	function upgradeToV2() public onlyAdmin {
		...
		for (j=0; j<5; j++) {
			if (address(_rt[j])!=address(0)) {
				_rtTradeEnable[j]=_rtEnable[j];			//initialize new variable for existing tokens
			} else {
				_rtEnable[j]=0;
				_rtTradeEnable[j]=0;
			}
